...
class FinancingTransactionAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val language = UserUtils.getInstance(context).language


        val transactionType = if (language == "ar") transaction?.CLASS_NAME_ARAB else transaction?.CLASS_NAME_ENG
        val currencyName = if (language == "ar") transaction?.CURRENCY_ARAB else transaction?.CURRENCY_ENG


        view.findViewById<TextView>(R.id.tvTransactionType).text = transactionType
        ...
        view.findViewById<TextView>(R.id.tv_currency_name2).text = currencyName
