...
class ComplaintViewModel @Inject constructor(private val appRepository: AppRepository) : BaseViewModel() {

    val complaint = Complaint()

    fun getClientSpinnerItems(context: Context): ArrayList<SpinnerItem> =
            arrayListOf(Title(context.getString(R.string
    ...
    fun getComplaintTitles(): LiveData<Resource<ArrayList<Title>>> =
            appRepository.complaintTitles

    fun sendComplaint(): LiveData<Resource<Complaint>> =
            appRepository.sendComplaint(complaint)
}